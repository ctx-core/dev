...
import { _AUTH0_DOMAIN } from '@ctx-core/auth0/lib.mjs'
import {
...
export async function patch__user__v2__auth0(ctx, form) {
	...
	const url = `https://${_AUTH0_DOMAIN(ctx)}/api/v2/users/${user_id}`
	const promise =
		fetch(
			url,
			{
...
export async function get__users_by_email__v2__auth0(ctx) {
	...
	const token__auth0 = await _token__auth0__management(ctx)
	const Authorization = _authorization__header__access_token__verify({ token__auth0 })
	const url = `https://${_AUTH0_DOMAIN(ctx)}/api/v2/users-by-email?email=${encodeURIComponent(email)}`
	const promise =
		fetch(
			url,
			{
...
export function _body__client_credentials__management(ctx) {
	const client_credentials = {
		...
		audience: `https://${_AUTH0_DOMAIN(ctx)}/api/v2/`
	}

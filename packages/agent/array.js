...
import {
	...
	last__array,
	compact__array
} from '@ctx-core/array/lib.js'
import { ensure__agent } from './lib.js'
import { log, debug } from '@ctx-core/logger/lib.js'
const logPrefix = '@ctx-core/agent/array.js'
/**
...
export function ensure__agent__array(ctx, ...ARR__ctx__agent) {
	...
	function pop(...keys__pop) {
		...
		const agent = this
		if (!keys__pop.length) keys__pop = [agent.scope__0]
	...
	function remove(...__ctx__remove) {
		...
		const agent = this
		const ctx__remove = clone__concat__array(...__ctx__remove)
		const ctx__set = _ctx__set()
		agent.set(ctx__set)
		return agent
		function _ctx__set() {
			const { scope } = agent
			const ctx__set = agent.pick()
			for (let i = 0; i < scope.length; i++) {
				const scope__ = scope[i]
				const value = ctx__remove[scope__]
				if (value) {
					const $ = ctx__set[scope__] || []
					ctx__set[scope__] =
						_difference__array($, value)
				}
			}
			return ctx__set
		}

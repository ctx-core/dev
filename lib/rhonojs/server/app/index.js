...
/**
 * Is `middleware` a Hono instance?
 *
 * `instanceof` alone is not enough: when hono is resolved from more than one
 * node_modules path the class identities differ, so a real Hono instance can
 * fail the check. Fall back to duck-typing on the two methods this module
 * actually calls.
 * @param {unknown}middleware
 * @returns {boolean}
 */
export function hono__is(middleware) {
	return middleware instanceof Hono
		|| (middleware != null
			&& typeof (/** @type {any} */(middleware)).fetch === 'function'
			&& typeof (/** @type {any} */(middleware)).route === 'function')
}

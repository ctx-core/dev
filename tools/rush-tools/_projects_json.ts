...
export async function _projects_json() {
	const projects_json = []
	projects_json.push(...(await promisify(glob)('packages/*')).map(package_path=>
		_project_json(package_path, 'packages', 'production'))
	)
	projects_json.push(...(await promisify(glob)('tools/*')).map(package_path=>
		_project_json(package_path, 'tools', 'production'))
	)

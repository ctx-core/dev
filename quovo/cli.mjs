...
async function mfa__cli() {
	...
	const brokerage__quovo = find__brokerage__quovo()
	...
	function prompt__confirm__new__account() {
		...
		const brokerage_name = brokerage__quovo.name
		if (!username) {
...
async function assign__account__cli__ctx__cli(opts) {
	...
	const set__options = opts.options.set
	const delete__options = opts.options.delete
...
function find__brokerage__quovo() {
	return ctx__cli.brokerages__quovo.find(
		brokerage__quovo =>
...
async function assign__user__ctx__cli(opts) {
	...
	if (!user__quovoname) {
		...
		const user_id__quovo = parseInt(choice__value(choice__user) || 0)
